...
function complexConvert(data) {
  ...
  for (const key in parsedData) {
    // Перевіряємо, чи ключ є властивістю об'єкта (не прототипом)
    if (parsedData.hasOwnProperty(key)) {
      // Отримуємо значення за поточним ключем
      const value = parsedData[key];

      // Перевіряємо, чи значення є числом
      if (typeof value === 'number') {
        // Збільшуємо числове значення на 1 і присвоюємо його в новий об'єкт
        result[key] = value + 1;
      } else if (typeof value === 'object' && value !== null) {
        // Якщо значення є об'єктом, рекурсивно застосовуємо функцію для обробки вкладеного об'єкта
        result[key] = complexConvert(JSON.stringify(value));
      } else {
        // Якщо значення не є числом або об'єктом, просто копіюємо його у новий об'єкт без змін
        result[key] = value;
      }
    }
  }
  // Повертаємо оброблений об'єкт у вигляді JSON-строки
  ...
  if (urlObj.search) {
    ...
    for (const [key, value] of existingParams.entries()) {
      if (paramsObj.hasOwnProperty(key)) {
        existingParams.set(key, paramsObj[key]);
...
function deleteSearchParams(keys, url) {
  ...
  // Перевіряємо, чи є параметри пошуку в URL
  if (urlObj.search) {
    // Розбираємо параметри пошуку в об'єкт URLSearchParams
    const searchParams = new URLSearchParams(urlObj.search);

    // Видаляємо вказані ключі з параметрів пошуку
    for (const key of keys) {
      searchParams.delete(key);
    }

    // Оновлюємо параметри пошуку в URL
...
function modifyURLParameters(url, params) {
  ...
  // Якщо параметр вже існує, метод set замінює його новим значенням.
    if (params.hasOwnProperty(key)) {
      ...
      // Використовуємо метод set для додавання або заміни параметра пошуку
      urlObj.searchParams.set(key, value);
    }
  }
...
function processUrl(url, options) {
  ...
  if (options.searchParams) {
    // Перебираємо параметри пошуку за допомогою циклу 'for in'.
    for (const key in options.searchParams) {
      if (options.searchParams.hasOwnProperty(key)) {
        // Додаємо кожний ключ та відповідне значення до об'єкта 'urlObj' за допомогою методу 'append'.
...
function manipulateQuery(url, options) {
  ...
  if (options.has("delete")) {
    ...
    for (const keyToDelete of options.get("delete")) {
...
function getUrlData(url) {
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
  const urlObj = new URL(url);
  // Витягуємо потрібні дані та формуємо об'єкт.
  const data = {
